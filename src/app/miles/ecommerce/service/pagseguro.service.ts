...
export class PagseguroService {
  ...
  public on_finish_order:Subject<any> = new Subject<any>();
  constructor(
  ...
  getSessionID(){
    return this.requisicao_service.get('pagseguro.get.sessionid');
  }

  getBandeirasCartaoCredito(){
    this.getSessionID().subscribe( (r:any) => {
      PagSeguroDirectPayment.setSessionId( r.dados );
  ...
  finalizarPedidoCartao(){
    ...
    let email                 = this.local_storage.get('_user').email;
    let valor_total           = parcelamento.split("^")[1];

    this.requisicao_service.get('pagseguro.set.pagamentocartaocredito',{
      cpf:cpf,
      hash:identificador,
      ...
      email:email,
      cliente:this.local_storage.get('_user_id'),
      cart_session:this.local_storage.get('_cart').sessionid
    })
    .subscribe(
      (_res:any) => {
        this.on_finish_order.next(_res);
      }
    );
		//  $.ajax({
		//  	type:"POST",
		//  	url:miles.api,
		//  	data:{
		//  		service:"pagseguro/set/pagamentocartaocredito",
